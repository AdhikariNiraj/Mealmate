...
class _RecipeEditScreenState extends State<RecipeEditScreen> {
  ...
  final String imgBBApiKey = "dc974a541a11cb6ce4c4efa69b764165";
  ...
  Future<String?> _uploadImageToImgBB() async {
    if (kIsWeb) {
      ...
      try {
        ...
        request.files.add(
          http.MultipartFile.fromBytes('image', _imageBytes!, filename: 'upload.jpg'),
        );
  ...
  Future<void> _saveRecipe() async {
    ...
    try {
      ...
      final uploadedImageUrl = await _uploadImageToImgBB();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title:
        Text(_editingRecipeId == null ? 'Create Recipe' : 'Edit Recipe'),
      ),
      body: _isLoading
          ? const Center(child: CircularProgressIndicator())
          : SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              CustomButton(
                onPressed: _saveRecipe,
                isLoading: _isLoading,
                text: _editingRecipeId == null ? 'Create Recipe' : 'Save Recipe',
              ),
            ],
          ),
        ),
